window.onload = function () {
  ...
  const inputList = document.querySelector('.recordList');
  ...
  function insertTodo(text) {
    ...
    if (!DATA[currentDate]) {
      DATA[currentDate] = [];
      DATA[currentDate].push(recordObj);
    } else {
      DATA[currentDate].push(recordObj);
    }
    ...
    // recordObj에 id값을 114번 줄에서 넣어주면 DATA[currentDate].length 값을 찾아올 수 없기 때문에 push해준 후 에 추가하여 local에 저장한다.
    recordObj.id = DATA[currentDate].length;
    save();
    inputBox.value = '';
  }
  ...
  function redrawLi() {
    ...
    for (let recordList in DATA) {
      if (recordList === currentDate) {
        for (let i = 0; i < DATA[recordList].length; i++) {
          ...
          spanEl2.innerHTML = DATA[recordList][i].record;
          liEl2.appendChild(spanEl2);
          liEl2.appendChild(delBtn2);
          inputList.appendChild(liEl2);
          liEl2.setAttribute('id', DATA[recordList][i].id);
  ...
  // 다음달,이전달 다른날, 첫 로드 될 때 마다 record 목록이 있으면(if로 조건문 처리) 다 지우고 다시 그려주는 함수 
  function resetInsert() {
    ...
    if (storeObj !== null) {
      ...
      // forEach로 작성되있는 모든 recordlist의 항목들을 돌면서 로컬에 저장되어 있는 목록을 화면에 만들어준다.
      parsed.forEach(function (record) {
        if (record) {
          ...
          lili.setAttribute('id', record.id);
          spanspan.innerHTML = record.record;
  ...
  function delWork(e) {
    ...
    // DATA[currentDate]를 filter함수를 이용해 record로 돌면서 record의 아이디값과 현재 내가 누른 아이디값이 같지 않은 것을 배열에 담아 리턴해주어서 
    // 내가 지우고자 하는 요소를 뺀 나머지 요소를 배열에 담아 리턴해준다. 
    // 그 배열을 다시 DATA[currentDate]에 할당하여 save();를 통해 localStorage에 넣어준다.
    const cleanToDos = DATA[currentDate].filter(function (record) {
      return record.id !== parseInt(delParentLi.id);
    });
